...
1. **首次启动向导**：选择一个文件夹作为文献库 → 填大模型 API Key（不填也能阅读与管理，只是没有 AI 功能）。向量引擎（llama.cpp + bge-m3）与 marker 解析环境自动下载配置（Windows CUDA / macOS Metal·MPS / 低端 CPU）
...
- 向量模型（bge-m3）、重排模型（bge-reranker-v2-m3）与 marker 解析环境均在首次使用时下载到本地缓存，全部本机推理（Windows CUDA / macOS Metal·MPS 自动加速）
