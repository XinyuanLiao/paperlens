...
- **检索**：段落级向量 + BM25 混合召回（RRF 融合），文献级标题/首页向量兜底，相关文献不漏

### 通用
- Claude 风格界面：暖色纸面 + 陶土红点缀，深 / 浅色主题跟随系统
- 全局字体自定义（中文 / 英文分别选择本机字体）+ 对话字号调节
- `Ctrl/Cmd+K` 命令面板；模型 / 思考等级 / 检索范围在界面内随时切换
- 多服务商配置：智谱 / DeepSeek / 通义 / Kimi / OpenAI / 任意 OpenAI 兼容接口 / Ollama
...
1. **首次启动向导**：选择一个文件夹作为文献库 → 填大模型 API Key（不填也能阅读与管理，只是没有 AI 功能）→ 选择嵌入模型（默认本地 `multilingual-e5-small`，自动下载）
...
- 嵌入模型默认本机推理；配合 Ollama 可实现完全离线使用
